...
import { Plus, MessageSquare, Download, RotateCcw, X, Loader2, MapPin } from "lucide-react";
...
export function PPPoECustomers() {
    ...
    const handleLocationSelect = (lat: number, lng: number, address?: string) => {
        setFormData(prev => ({
            ...
            // Optionally update location if address is provided and location is empty
            location: prev.location || (address ? address.split(',').slice(0, 2).join(',').trim() : prev.location)
        }));
    };
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            <input
                                                ...
                                                className="flex-1 px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-slate-200 placeholder:text-slate-500 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                            />
                                            <button
                                                type="button"
